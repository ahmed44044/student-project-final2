...
import ScientificModel from "../../../../DB/model/ScientificReference.model.js";
    ...
    async(req,res,next)=>{
        ...
        if (Degree<=100) {
            const course= await findOne({model:materialModel,
                filter:{course:subjectName ,createdBy:studentID},
            
            })
            if (!course) {
              return  next(new Error('sorry,student not register this course',{cause:400}))
            } else {
            ...
            return  next(new Error('sorry,max of Degree 100',{cause:400}))
        }
    ...
    async(req,res,next)=>{
        ...
        if (Degree <=100) {
            ...
            const updateRes= await findOneAndUpdate({model:resultModel,
                filter:{_id:id},
            data:{Degree}})
            return  updateRes ? res.status(201).json({message:"done"}) :  next(new Error(`fail to update result `,{cause:400 }))
        } else {
            return  next(new Error('sorry,max of Degree 100',{cause:400}))
        }
        }
...
export const addScientificReference= asyncHandler(
    async(req,res,next)=>{
        const {LinkReference,courseName}=req.body
        const admin = await findOne({
            model:userModel,
            filter:{_id:req.user._id}
        })
        if (admin) {
            const addScientificReference = await create({
                model:ScientificModel,
                data:{LinkReference,courseName}
            })
            return  addScientificReference ? res.status(201).json({message:"done",addScientificReference}) : next(new Error('fail to add Scientific Reference ',{cause:400}))
        } else {
            return next(new Error('u can not add link ',{cause:409}))
        }
    }
)