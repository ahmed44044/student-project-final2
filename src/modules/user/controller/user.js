...
async(req,res,next)=>{
    ...
    filter:{role:'User'},
select:'userName level DepartmentName NationalID'})
...
export const getAllCourses= asyncHandler(
    async(req,res,next)=>{
        ...
        const allCourse= await find({model:userModel,filter:{_id:id},
            select:'userName DepartmentName level NationalID'
            ,populate:[
            {
                path:'material',
...
export const ShowReference= asyncHandler(
    async(req,res,next)=>{
      const Reference= await find({model:ScientificModel,
    filter:{}})

        Reference.length?  res.status(201).json({message:"done",Reference}) : next(new Error('u not student or u not register courses ,plz add courses first '))
    }
)